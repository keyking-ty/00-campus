...
<script>
    ...
    layui.use(['layer', 'form','table'], function(){
        ...
        $("#choseIcon").click(function(){
            layer.open({
                type: 2,
                area:['95%', '95%'],
                content:'${request.contextPath!}/static/page/systemSetting/icons.html',
                shadeClose:true,
                end: function(){
                    $("#icon").select();
                }
	...
	function initOps(data) {
		var html = "";
		if (data && data != ''){
			var ids = data.split(',');
			allOps.forEach(function (opValue) {
				if (checkId(ids,opValue.id)){
					html +='<input type="checkbox" name="operates" title= "' + opValue.name+'" value="' + opValue.id + '" checked="checked">';
				}else{
					html +='<input type="checkbox" name="operates" title= "' + opValue.name+'" value="' + opValue.id + '">';
				}
	        });
		}else{
	        allOps.forEach(function (opValue) {
    ...
    function pageInit() {
        ...
        function beforeEditName(treeId, treeNode) {
            ...
            layer.confirm("进入节点 -- " + treeNode.name + " 的编辑状态吗？", {icon: 3, title: '提示信息'}, function (index) {
            	layer.close(index);
            	//根据id获取菜单节点对象的数据并填充进编辑的表单中
                $.ajax({
                    type: "POST",
                    url:"${request.contextPath!}/admin/menu/selectMenuById",
                    data:{id:treeNode.id},
                    async: false,
        ...
        function beforeRemove(treeId, treeNode) {
            ...
            var zTree = $.fn.zTree.getZTreeObj("treeDemo");
            zTree.selectNode(treeNode);
            layer.confirm("确认删除 节点 -- " + treeNode.name + " 吗？", {icon: 3, title: '提示信息'}, function (index) {
            	layer.close(index);
            	$.ajax({
	                type: "POST",
	                url: "${request.contextPath!}/admin/menu/deletemenu",
	                data: {id: treeNode.id},
	                async: false,
                    complete: function(jqXHR, textStatus){
                        if (jqXHR.status == 401){
        ...
        layui.form.on('submit(addeditsubmitfilter)', function(data) {
            ...
            $.ajax({
                type: "POST",
                url:"${request.contextPath!}/admin/menu/addupdatemenu",
                data:_data,
                async: false,
                dataType: 'json',
                complete: function(jqXHR, textStatus){
        ...
        $.ajax({
            type: "POST",
            url:'${request.contextPath!}/admin/menu/loadCheckMenuInfo?parentId=-1',
            async: false,
            dataType: 'json',
            timeout: 1000,
            cache: false,
...
<div id="addeditformdivid" hidden="" class="layui-fluid" style="margin: 15px;">
    <form class="layui-form" action="" id="addeditformid">
        ...
        <div class="layui-form-item">
            ...
            <div id="operateDivId" class="layui-input-block">
            </div>
        </div>
        <div class="layui-form-item">
