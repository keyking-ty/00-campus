...
<html lang="cn">
	<head>
		...
		<script type="text/javascript">
	        ...
	        function pageInit() {
	            $("#delete").hide();
	            checkOperate('${request.contextPath!}',6501,false);
	            //创建jqGrid组件
	            jQuery("#list2").jqGrid({
	                mtype: "post",//向后台请求数据的ajax的类型。可选post,get
	                url: '${request.contextPath!}/admin/goods/list',
	                ...
	                colModel: [
	                    ...
	                    {label: '类型', name: 'type', width: 180, sortable: false,align:'center',stype: 'select',
	                		searchoptions: {
	                			dataUrl: '${request.contextPath!}/admin/search/clients?ops=失物招领,寻物启事',
	                			sopt: ['eq','ne']
	                		}
	                	},
	            ...
	            $("#delete").click(function () {
	                ...
	                layer.open({
	                    ...
	                    yes: function(index, layero){//点击是时候的回调
	                        ...
	                        $.ajax({
	                            type: "POST",
	                            url:"${request.contextPath!}/admin/goods/delete",
	                            data:{id: selectId},
	                            async: false,
	            ...
	            $("#look").click(function () {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url:"${request.contextPath!}/admin/goods/selectById",
	                    data:{id: selectId},
	                    async: false,
						complete: function(jqXHR, textStatus){
							if (jqXHR.status == 401){
	                    ...
	                    success: function(data) {
	                        ...
	                        //向表单填充数据
	                        $("#type").html('类型: '  + data.goods.type);
	                        $("#userName").html('用户: '  + data.goods.userName);
	                        $("#content").html('内容: ' + data.goods.content);
	                        $("#schoolName").html('学校: '  +  data.goods.schoolName);
	                        $("#time").html('发布时间: '   +  data.goods.time);
	                        fillPics(data.goods.pics);
				            layui.form.render();
